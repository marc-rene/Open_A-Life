...
void Get_Set_Global_Settings()
{
	...
	if(Open_Config() )
	{
		...
		Global_settings.LOG_LEVEL			= Get_Value_From_Settings(	"MAXIMUM RUNTIME LOG LEVEL"	);
		...
		Global_settings.File_ReOpen_Attempts		= Get_Value_From_Settings(	"HOW MANY FILE REOPEN ATTEMPTS"	);
		Global_settings.USE_HIGH_PERFORMANCE_MODE	= Get_Value_From_Settings(	"...Gotta go fast? Some extra text");
