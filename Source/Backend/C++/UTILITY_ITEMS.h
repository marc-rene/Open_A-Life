...
void Check_Work_Permission_For_Faction()
{
	while (!Exiting)
	{
		...
		{	
			...
			{
				ALL_FACTIONS.at(i).TAKE_A_BREAK(SPEED_DIVIDER*1000);
			}	
		}
...
{
	std::vector<Tile>	world_copy(WORLD_SIZE);
	...
	while (true)
	{
		...
		for (uS i = 0; i < WORLD_SIZE; i++)
		{
			...
			IF_LOW_PERFORMANCE { NAP(SPEED_DIVIDER*100);	}
		}
		
		IF_HIGH_PERFORMANCE	{	Permission_to_work = true;	}
		IF_LOW_PERFORMANCE	{	NAP(SPEED_DIVIDER*50);	}
		...
		for (uS i = 0; i < WORLD_SIZE; i++)
		{
			...
			//IF_HIGH_PERFORMANCE	{	NAP(SPEED_DIVIDER*5);		}
			IF_LOW_PERFORMANCE	{	NAP(SPEED_DIVIDER*500);	}
		}
		...
		IF_HIGH_PERFORMANCE	{	SLEEP(SPEED_DIVIDER*3);	}
		IF_LOW_PERFORMANCE	{	SLEEP(SPEED_DIVIDER*15);	}
		
		Permission_to_work = true;
		
		HIGH_LOG
			printf("\nDone writing to world file");
		END_LOG
	}
}
