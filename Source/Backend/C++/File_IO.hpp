...
void Check_ToDo_File()
{
	char ToDo_file_path[FILE_PATH_BUFFER_SIZE];
	

	Get_File_Path(Global_settings.API_Folder_Path, WHAT_TODO_FILE, ToDo_file_path);


	std::string path_string = ToDo_file_path;

	LOG( std::string("Attempting to open todo file from " + path_string ) );

	/// Lets get the file now!
	Global_settings.What_Todo_Ptr = fopen(ToDo_file_path, "r");


	IF_HIGH_PERFORMANCE
	{
		if( Check_File_Is_Open(Global_settings.What_Todo_Ptr, ToDo_file_path, "r", 1, false) )
			{	LOG("All's good opening What.ToDo", BORING );	}

		else
			{	LOG("What.ToDo won't open... crap", SEVERE_ERROR );	}
	}//end if high performance

	IF_LOW_PERFORMANCE
	{ 
		if( Check_File_Is_Open(Global_settings.What_Todo_Ptr, ToDo_file_path, "r", 5, false) )
			{	LOG("All's good opening What.ToDo", BORING );	}
	...
	char file_contents[32];	// 32 is overkill
	std::string int_part;	// Its a string so i can use stoi

	uShort 	eof_index = 0;
	chunky 	temp_long_int;
	chunky	time_stamp;
	...
	do
	{
		...
		{
			file_contents[i] = fgetc(Global_settings.What_Todo_Ptr);

			eof_index = i;

			if (feof(Global_settings.What_Todo_Ptr) ) 	// Incase we reach the end
			{
				file_contents[i] = '\0'; 	// Super important
				break;		
			}
		}
		...
		/*	From https://github.com/marc-rene/Open_A-Life/blob/main/README.md
		...
		*/
		...
		switch (file_contents[0])
		{
			case 'C':	// Continue on unless the time is out of wack
				Global_settings.Permission_to_run = true; 	// Incase this ever gets paused and set to false
				for (uMint i = 1; i < 32; i++)
				{
					int_part[ (i-1) ] = file_contents[i];
				...
				LOG( std::string("Reading todo file TIME STAMP: " + int_part ), no_importance );
				temp_long_int = std::stoi(int_part);
				
				//time_stamp > temp_long_int
				time_stamp = time(NULL);	//	Get the current time in seconds

				temp_long_int = time_stamp - temp_long_int; // Whats the difference between now and what was read?

				if (temp_long_int > Global_settings.Unix_Time_Read_Leeway)
				{
					LOG("THERES DIFFERENCE IN TIMESTAMPS WHEN READING TODO FILE, EXCITING", IMPORTANT);
					SLEEP(1);
					//exit(67);
					break; // incase exit doesn't work
				}
				
				break;
			
			
			case 'A':	// https://www.youtube.com/watch?v=qsvbYKr19ng
				Global_settings.Permission_to_run = true; 	// Incase this ever gets paused and set to false
				break;
			...
			case 'S':	// Go SLOW
				Global_settings.Permission_to_run = true; 	// Incase this ever gets paused and set to false
				Global_settings.SPEED_DIVIDER = 250;
				Global_settings.Unix_Time_Read_Leeway = 65000;
				Global_settings.USE_HIGH_PERFORMANCE_MODE = false;
				
				LOG("Going SUPER SLOW as dictated by What.ToDo", IMPORTANT);

				break;
			

			case 'F':	// Go back to normal
				Global_settings.Permission_to_run 	= true; 	// Incase this ever gets paused and set to false
				...
				// Lets Change it to continue now, so everyone knows the changes have been reverted
				fclose(Global_settings.What_Todo_Ptr);
				Global_settings.What_Todo_Ptr = fopen(ToDo_file_path, "w"); 	
				
				// Lets write
				fputc('C', Global_settings.What_Todo_Ptr);
				fputc(' ', Global_settings.What_Todo_Ptr);
				putw( time(NULL), Global_settings.What_Todo_Ptr); 	// Lets write in the current time in seconds
				// fclose is called later on anyway, no need to recall it
			...
			case 'T':
				...
				LOG( std::string("Reading todo file SPEED DIVIDER SET: " + int_part ), no_importance );
				temp_long_int = std::stoi(int_part);

				Global_settings.SPEED_DIVIDER = temp_long_int;
				
				break;

			
			case 'P':
				LOG("WHOLE BACKEND IS PAUSED", IMPORTANT, no_error, true);
				Global_settings.Permission_to_run = false;
				break;
			...
			case 'U':
				...
				LOG( std::string("Pausing for X seconds: " + int_part ), no_importance );
				temp_long_int = std::stoi(int_part);

				Global_settings.Permission_to_run = false;
				std::this_thread::sleep_for(std::chrono::seconds(temp_long_int));
				Global_settings.Permission_to_run = true;

				break;


			default:						//	WTF?
				LOG("...ERROR in What to do switch part in File_io.hpp", MINOR_ERROR);
				Global_settings.Permission_to_run = true; 	// Incase this ever gets paused and set to false
				break;
		...
		fclose(Global_settings.What_Todo_Ptr); // gotta close and let changes happen and be read
...
//	Verified works
void Log_To_File(const char* message, bool include_time, bool new_line)
{
	if (include_time)
	{
		//	-	-	-	-	-	-	-	-	-	-	-	-	-	-	-	-	-	-	-	-	-	-
...
{
	Log_To_File(message, include_time, true);
}

void Log_To_File(const char* message)
{
	Log_To_File(message, true, true);
}
...
void Log_To_File(std::string message)
{
	...
	Log_To_File(charred_message, true, true);
}
